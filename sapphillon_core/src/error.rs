...
/// trace are preserved as a plain string — `deno_core` types never cross the
/// ABI boundary, so we capture them as text inside `sapphillon_deno`.
#[derive(Debug, Error)]
#[error("{error_type:?}: {message}")]
pub struct WorkflowRuntimeError {
    pub message: String,
    pub error_type: WorkflowRuntimeErrorType,
    /// Full JS error message + stack trace formatted by `sapphillon_deno`.
